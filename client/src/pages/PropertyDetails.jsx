...
import { format, startOfDay, isWithinInterval, addDays } from 'date-fns';
        ...
        <div className="lg:col-span-2 space-y-5">
          {[100, 80, 140, 180, 220].map((h, i) => (
            <div
              key={i}
              style={{ height: h }}
              className="bg-gray-200 dark:bg-gray-700 rounded-2xl"
            />
          ))}
        </div>
...
const ImageGallery = ({ images }) => {
  ...
  if (images.length === 1) {
    return (
      <div
        className="relative aspect-[16/9] rounded-2xl overflow-hidden shadow-lg group cursor-pointer"
        onClick={() => openModal(0)}
      >
        <OptimizedImage
          src={images[0]}
  ...
  return (
    <div className="relative">
      ...
      {/* ── Mobile single image ── */}
      <div
        className="md:hidden relative aspect-[4/3] rounded-2xl overflow-hidden shadow-md cursor-pointer"
        onClick={() => openModal(0)}
      >
        <OptimizedImage
          src={images[0]}
              ...
              {images.length > 1 && (
                <>
                  <button
                    onClick={prev}
                    aria-label="Previous"
                    className="absolute left-4 w-11 h-11 rounded-full bg-white/10 hover:bg-white/25 flex items-center justify-center text-white transition-colors"
                  >
                    <ChevronLeft size={22} />
                  </button>
                  <button
                    onClick={next}
                    aria-label="Next"
                    className="absolute right-4 w-11 h-11 rounded-full bg-white/10 hover:bg-white/25 flex items-center justify-center text-white transition-colors"
                  >
...
const QuickStats = ({ property }) => {
  ...
  const items = [
    ...
    { icon: Users, label: `Up to ${property.maxGuests || 4} guests` },
    {
      icon: Maximize2,
      label:
        (property.propertyType || 'Property').charAt(0).toUpperCase() +
        (property.propertyType || 'Property').slice(1),
    },
  ];
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
      {items.map((item) => (
        <div
          key={item.label}
          className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700/60 rounded-xl px-4 py-3"
        >
...
const BookingCard = ({ property }) => {
  ...
  const isBooked = (day) =>
    futureRanges.some((r) => isWithinInterval(startOfDay(day), { start: r.from, end: r.to }));
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 text-center">
        ...
        <div className="text-left bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 mb-5 space-y-2.5 text-sm">
          {[
            [
              'Check-in',
              new Date(bookingResult.checkIn).toLocaleDateString('en-PK', {
                weekday: 'short',
                day: 'numeric',
                month: 'long',
              }),
            ...
            [
              'Check-out',
              new Date(bookingResult.checkOut).toLocaleDateString('en-PK', {
                weekday: 'short',
                day: 'numeric',
                month: 'long',
              }),
          ...
          <div className="border-t border-gray-200 dark:border-gray-600 pt-2.5 flex justify-between font-bold">
            ...
            <span className="text-earth-brown">
              Rs {bookingResult.totalPrice?.toLocaleString()}
            </span>
          </div>
        </div>
        ...
        >
          <div className="grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-600">
            <div className="px-4 pt-3 pb-2.5">
              ...
              <p
                className={`text-sm font-semibold ${range.from ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}
              >
                {checkInStr}
              </p>
            </div>
            <div className="px-4 pt-3 pb-2.5">
              ...
              <p
                className={`text-sm font-semibold ${range.to ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}
              >
                {checkOutStr}
              </p>
            </div>
              ...
              <div className="rdp-rentifi px-2 pb-3">
                <DayPicker
                  ...
                  classNames={{
                    ...
                    button_previous:
                      'absolute left-1 w-7 h-7 rounded-lg flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-gray-600 dark:text-gray-400',
                    button_next:
                      'absolute right-1 w-7 h-7 rounded-lg flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-gray-600 dark:text-gray-400',
                    month_grid: 'w-full border-collapse',
                    weekdays: 'flex',
                    weekday:
                      'flex-1 text-center text-[11px] font-medium text-gray-400 dark:text-gray-500 py-1',
                    ...
                    range_middle:
                      '!bg-[#A0937D]/15 dark:!bg-[#A0937D]/25 !text-gray-800 dark:!text-gray-200 !rounded-none',
                    today: 'font-bold text-earth-brown underline underline-offset-2',
                    disabled:
                      '!text-gray-300 dark:!text-gray-600 !cursor-not-allowed line-through !bg-red-50 dark:!bg-red-900/10 rounded-full',
          ...
          >
            <div className="border-t border-gray-100 dark:border-gray-700 mx-5 mb-5 pt-4 space-y-2.5 text-sm">
              <div className="flex justify-between text-gray-600 dark:text-gray-400">
                <span>
                  Rs {property.price?.toLocaleString()} × {pricing.nights} night
                  {pricing.nights !== 1 ? 's' : ''}
                </span>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4">
        <div className="text-center">
          ...
          <Link
            to="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-earth-brown text-white rounded-xl font-medium text-sm hover:bg-earth-brown/90 transition-colors"
          >
        ...
        <main className="flex-grow pt-20 pb-16">
          <div className="page-container py-8">
            ...
            >
              ...
              >
                <Link to="/" className="hover:text-earth-brown transition-colors">
                  Home
                </Link>
                <ChevronRight size={12} />
                <Link to="/properties" className="hover:text-earth-brown transition-colors">
                  Properties
                </Link>
                <ChevronRight size={12} />
                <span className="text-gray-700 dark:text-gray-300 font-medium truncate max-w-[220px]">
                  {title}
                </span>
              </nav>
              ...
              <div className="space-y-6 min-w-0">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link
                      to={`/host/${property.host?._id || property.host}`}
                      className="font-semibold text-gray-900 dark:text-white text-sm hover:text-earth-brown transition-colors truncate block"
                    >
                ...
                >
                  ...
                  <p className="text-gray-600 dark:text-gray-300 leading-[1.75] text-[15px]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {property.amenities.map((amenity, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors group"
                          >
                      ...
                      <p className="text-xs mt-1 text-gray-500 dark:text-gray-400">
                        {property.isLocationVerified ? (
                          <span className="text-green-600 dark:text-green-400 flex items-center gap-1">
                            <CheckCircle size={10} /> Location verified by host
                            {property.locationCapturedAt
                              ? ` · ${new Date(property.locationCapturedAt).toLocaleDateString()}`
                              : ''}
                          </span>
                        ) : (
                ...
                >
                  ...
                  <Link to={`/host/${property.host?._id || property.host}`} className="block group">
                    <div className="flex items-start gap-5 p-5 rounded-xl bg-gradient-to-br from-earth-brown/5 to-soft-peach/10 dark:from-earth-brown/10 dark:to-gray-700/40 group-hover:from-earth-brown/10 group-hover:to-soft-peach/20 transition-all duration-300">
                      ...
                      <ArrowRight
                        size={16}
                        className="text-gray-300 dark:text-gray-600 group-hover:text-earth-brown group-hover:translate-x-1 transition-all shrink-0 mt-1"
                      />
                    </div>
                  </Link>
                </motion.section>
